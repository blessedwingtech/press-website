...
export default async function SubMenuCategoryPage({ params }: SubMenuPageProps) {
  ...
  // Récupérer les articles associés à cette sous-catégorie spécifique
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-grow w-full">
      ...
      <div className="flex items-center gap-1.5 my-4 text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">
        ...
        <ChevronRight className="w-3 h-3 text-slate-600" />
        <span className="text-slate-200">{submenu.nom}</span>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <span className="text-xs text-slate-500 font-semibold">{articles.length} article(s) trouvé(s)</span>
      </div>
                ...
                >
                  <Link href={`/articles/${article.slug}`} className="block relative h-48 overflow-hidden bg-slate-955">
                    ...
                    <img
                      ...
                      alt={article.titre}
                      className="w-full h-full object-cover transform group-hover:scale-103 transition-transform duration-350 select-none"
                    />
                  </Link>
                  <div className="p-5 flex-grow flex flex-col justify-between">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-855 flex justify-between items-center text-[10px] font-semibold text-slate-450">
                      ...
                      <Link
                        ...
                        className="text-emerald-400 hover:text-emerald-350 transition-colors"
                      >
                        Lire l'article
                      </Link>
                    </div>
