...
import { saveAd, deleteAd } from '@/app/admin/ads/actions';
import { Plus, Trash2, Edit2, Landmark, CheckCircle, XCircle, Upload, Loader2 } from 'lucide-react';
...
export default function AdminAdsClient({ ads }: AdminAdsProps) {
  const [adId, setAdId] = useState<string | undefined>(undefined);
  const [titre, setTitre] = useState('');
  const [imageUrl, setImageUrl] = useState('');
  const [lien, setLien] = useState('');
  const [position, setPosition] = useState<'header' | 'sidebar' | 'footer'>('sidebar');
  ...
  const handleEdit = (ad: AdData) => {
    ...
    setLien(ad.lien);
    setPosition(ad.position as any);
    setActive(ad.active);
    ...
    <div className="space-y-6 w-full">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">
          Ajoutez de nouvelles publicités, liez-les à des redirections de marques partenaires et affectez-les aux slots haut, bas ou latéraux du site.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-slate-900/20 border border-slate-800/80 rounded-xl p-5 shadow-2xl">
            ...
            {ads.length === 0 ? (
              <p className="text-xs text-slate-550 text-center py-8">Aucun affichage publicitaire enregistré dans la base.</p>
            ) : (
                ...
                {ads.map((ad) => (
                  ...
                  >
                    <div>
                      ...
                      <h3 className="font-bold text-white text-xs sm:text-sm line-clamp-1 leading-none">{ad.titre}</h3>
                      <p className="text-[10px] text-slate-500 truncate mt-1">
                        Lien: <a href={ad.lien} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">{ad.lien}</a>
                      </p>
                    </div>
                    ...
                    <div className="flex justify-between items-center border-t border-slate-800/50 pt-3 mt-3 text-xs">
                      <div className="flex items-center">
                        {ad.active ? (
                          <span className="text-[9px] font-extrabold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full flex items-center gap-1 select-none">
                            <CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> Activée
                          </span>
                        ) : (
                          <span className="text-[9px] font-extrabold text-slate-500 bg-slate-900 border border-slate-800 px-2 py-0.5 rounded-full flex items-center gap-1 select-none">
                            <XCircle className="w-3.5 h-3.5 text-slate-500" /> Désactivée
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex gap-1.5">
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-cyan-455 hover:bg-slate-850 rounded transition-colors"
                        ...
                        <button
                          onClick={async () => {
                            if (confirm(`Voulez-vous supprimer la publicité "${ad.titre}" ?`)) {
          ...
          <div className="bg-slate-900/20 border border-slate-800/80 rounded-xl p-5 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    <label className="flex-grow flex justify-center items-center gap-1.5 py-2.5 px-3 bg-slate-900 border border-slate-800 text-slate-200 text-xs font-bold rounded-lg hover:bg-slate-850 hover:text-white transition-colors cursor-pointer">
                      {uploading ? (
                        <>
                          <Loader2 className="w-3.5 h-3.5 animate-spin text-slate-200" /> Téléversement...
                        </>
                      ) : (
                        <>
                          <Upload className="w-3.5 h-3.5 text-slate-400" /> Charger un fichier local
                        </>
                      )}
                      <input
                        type="file"
              ...
              <div className="flex items-center gap-2 py-1 select-none">
                ...
                />
                <label htmlFor="active" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider cursor-pointer">
              ...
              <div className="flex gap-2">
                ...
                >
                  {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin text-slate-950" /> : <Plus className="w-3.5 h-3.5 text-slate-950" />}
